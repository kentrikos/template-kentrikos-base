...
module "vpc-endpoint-services-nlb-ingress" {
  ...
  k8s_ingress_service_nodeport = data.terraform_remote_state.application-account.outputs.ingress_service_nodeport_http
  k8s_workers_asg_names        = data.terraform_remote_state.application-account.outputs.workers_asg_names
